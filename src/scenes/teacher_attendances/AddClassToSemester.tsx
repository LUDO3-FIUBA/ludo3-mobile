...
interface AddSemesterRouteParams {
  semesterToBeAddedAClass: TeacherSemester;
}

const AddClassToSemester: React.FC = () => {
  ...
  const semester: TeacherSemester = (route.params as AddSemesterRouteParams).semesterToBeAddedAClass
        ...
        onPress={() => {
          if (startDate && finishDate && startTime && finishTime) {
            if (isFinishAfterStart(startDate, startTime, finishDate, finishTime)) {
              ...
              teacherEvaluationsRepository.create(semester, evaluationName, startFullDate, finishFullDate)
                .then(() => {
                  setCreating(false);
                  navigation.goBack();
                })
