...
const SecretaryForm: React.FC = () => {
  ...
  const route = useRoute<RouteProp<SecretaryFormRouteParams, 'SecretaryForm'>>();
  ...
  const handleSubmit = async () => {
    ...
    try {
      const data: Partial<Secretary> = {
        ...
        schedule,
        contactInfo,
        parentSecretary: existing?.parentSecretary ?? null,
      };
      if (existing) {
        await secretariesRepository.updateSecretary(existing.id, data);
        Alert.alert('Exito', 'Secretaría actualizada correctamente.');
      } else {
        await secretariesRepository.createSecretary(data);
        Alert.alert('Exito', 'Secretaría creada correctamente.');
      }
      navigation.goBack();
    } catch (error) {
      Alert.alert('Error', 'No se pudo guardar la secretaría. Intente de nuevo.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    ...
    >
      <ScrollView style={styles.container} contentContainerStyle={styles.content}>
        ...
        <RoundedButton
          text={saving ? 'Guardando...' : existing ? 'Guardar cambios' : 'Crear secretaría'}
          enabled={!saving}
